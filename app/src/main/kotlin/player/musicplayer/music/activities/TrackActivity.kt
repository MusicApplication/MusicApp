        ...
        activity_track_play_pause.setOnClickListener {
            ...
            Handler().postDelayed({
                if (!MusicService.getIsPlaying()) {
                    ...
                    if (!inAppMusicPauseInterstitial.getInstance().isShowPauseAd){
                        if (inAppMusicPauseInterstitial.getInstance().getInterstitialAd() != null) {
                            inAppMusicPauseInterstitial.getInstance().getInterstitialAd()?.fullScreenContentCallback = object : FullScreenContentCallback() {
                                override fun onAdDismissedFullScreenContent() {
                                    inAppMusicPauseInterstitial.getInstance().makeAdNull()
                                    Handler().postDelayed({
                                        if (inAppMusicPauseInterstitial.getInstance().getInterstitialAd() == null) {
                                            inAppMusicPauseInterstitial.getInstance().createAd(this@TrackActivity)
                                        }
                                    }, 540000)
                                    Handler().postDelayed({
                                        inAppMusicPauseInterstitial.getInstance().makeShowPauseAdFalse()
                                    }, 600000)
                                }
                            ...
                            inAppMusicPauseInterstitial.getInstance().getInterstitialAd()?.show(this)
                        }
                        inAppMusicPauseInterstitial.getInstance().makeShowPauseAdTrue()
                    }
